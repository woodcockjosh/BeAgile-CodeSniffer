...
class BeAgile_Sniffs_Commenting_VariableCommentSniff extends PHP_CodeSniffer_Standards_AbstractVariableSniff
{
    ...
    public function processMemberVar(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        if ($commentEnd !== false && $tokens[$commentEnd]['code'] === T_COMMENT) {
            ...
            if ($commentFor !== $stackPtr) {
                $phpcsFile->addError('Missing variable doc comment' . $commentFor, $stackPtr, 'Missing');
                return;
            }
        }
		...
		//get the name of the variable for which the comment is for
		$tokens = $phpcsFile->getTokens();
        $memberVarName     = ltrim($tokens[$stackPtr]['content'], '$');

        // Check each tag.
        $this->processVar($commentStart, $commentEnd,$memberVarName);
     ...
     * @return void
     */
    protected function processVar($commentStart, $commentEnd, $memberVarName)
    {
        ...
        if ($var !== null) {
            ...
            if (empty($content) === true) {
                ...
                if ($content != 'double' && $content !== $suggestedType) {
			...
			//see if the member variable is an object
			if(substr($memberVarName,0,1) == 'm' && substr($memberVarName,1,3) == 'obj')
			{
				//if the variable type is not a class name or an array then it must be
				if(in_array($content,array('double','string','float','integer','boolean')) || $content != 'array')
				{
					if(strtoupper(substr($content,0,1)) != substr($content,0,1) && PHP_CodeSniffer::isCamelCaps(substr($content,1)))
					{
						$error = 'variable type for $%s must be array or a class name';
						$data = array($memberVarName);
						$this->currentFile->addError($error, $errorPos, 'IncorrectVarType',$data);
					}
				}
			}
			
			//see if the member variable is an string
			if(substr($memberVarName,0,1) == 'm' && substr($memberVarName,1,3) == 'str' && $content != 'string')
			{
				$error = 'variable type annotation "str" for $%s does not match %s';
				$data = array($memberVarName,$content);
				$this->currentFile->addError($error, $errorPos, 'IncorrectVarType',$data);
			}
			
			//see if the member variable is an integer
			if(substr($memberVarName,0,1) == 'm' && substr($memberVarName,1,3) == 'int' && $content != 'integer')
			{
				$error = 'variable type annotation "int" for $%s does not match %s';
				$data = array($memberVarName,$content);
				$this->currentFile->addError($error, $errorPos, 'IncorrectVarType',$data);
			}
			
						
			//see if the member variable is an double
			if(substr($memberVarName,0,1) == 'm' && substr($memberVarName,1,3) == 'dbl' && $content != 'double')
			{
				$error = 'variable type annotation "dbl" for $%s does not match %s';
				$data = array($memberVarName,$content);
				$this->currentFile->addError($error, $errorPos, 'IncorrectVarType',$data);
			}
			
			//see if the member variable is an boolean
			if(substr($memberVarName,0,1) == 'm' && substr($memberVarName,1,4) == 'bool' && $content != 'boolean')
			{
				$error = 'variable type annotation "bool" for $%s does not match %s';
				$data = array($memberVarName,$content);
				$this->currentFile->addError($error, $errorPos, 'IncorrectVarType',$data);
			}
